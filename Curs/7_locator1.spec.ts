...
test('locator test', async()=>{
    ...
    //2. class name: works only with dot . (not 'class=' )
    ...
    const forgotPassLink:Locator = page.locator('text=Forgotten Password');
    const forgotPassLinkExist = await forgotPassLink.isEnabled();
    console.log(forgotPassLinkExist);

    //4. css: is not mandatory to write 'css='
    ...
    await email.fill("anca@gmail.com");
    await telephone.fill("1234567");
    await privacyCheckBox.click();

    //5. xpath:is not mandatory to write 'xpath='
